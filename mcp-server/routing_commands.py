...
TIER_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "t3": {
        ...
        "suitable_for": [
            "bug diagnosis", "planning", "architecture", "security review",
...
_COMPLEXITY_KEYWORDS: dict[str, int] = {
    # Debug & diagnosis
    "debug": 2, "diagnose": 2, "trace": 2, "root cause": 3, "regression": 2,
    "why": 1, "explain": 1, "investigate": 2,
    # Planning & architecture
    "plan": 3, "design": 3, "architect": 3, "strategy": 3, "roadmap": 3,
    "migrate": 3, "refactor": 3, "restructure": 3,
    # Security
    "security": 2, "vulnerability": 3, "exploit": 3, "audit": 2, "cve": 3,
    "penetration test": 4, "threat": 3,
    # Multi-step
    "multi-step": 2, "deploy": 2, "orchestrate": 2, "coordinate": 2,
    "pipeline": 2, "workflow": 2,
    # Analysis
    "analyze": 2, "evaluate": 2, "compare": 1, "synthesize": 2,
    "summarize": 1, "optimize": 2,
    # Domain-specific
    "multi-agent": 3, "rag": 2, "fine-tune": 3, "training": 3,
    "reinforcement learning": 4,
    # Risk indicators
    "production": 2, "critical": 3, "blocking": 2, "revenue": 2,
}

# Domains that always require at least T2
_TIER_TWO_MINIMUM_KEYWORDS: list[str] = [
    "security review", "auth", "authentication", "authorization",
    "database", "migration", "schema change", "data model",
    "api design", "contract", "protocol",
]

# Signal words for very simple tasks
_SIMPLE_INDICATORS: list[str] = [
    "format", "convert", "translate", "spell check", "paraphrase",
    "rename", "copy", "move", "list", "show", "simple",
    "one-step", "single", "trivial",
]
...
def _estimate_tool_count(task: str) -> int:
    ...
    # File operations
    files = len(re.findall(r'\b(read|open|examine|inspect|review)\s+(file|code|log|config)', task_lower))
    count += files
...
def _score_to_tier(score: int, features: list[str]) -> str:
    ...
    if any(kw in feature_text for kw in ["vulnerability", "exploit", "cve", "penetration test",
                                          "threat", "reinforcement learning"]):
...
def _classify_reasoning_depth(features: list[str]) -> str:
    ...
    if any(kw in feature_text for kw in ["plan", "design", "architect", "strategy",
                                          "roadmap", "multi-agent"]):
        return "multi-step"
    if any(kw in feature_text for kw in ["debug", "diagnose", "root cause", "investigate",
                                          "analyze", "evaluate", "synthesize"]):
        return "deep"
    if any(kw in feature_text for kw in ["explain", "why", "compare", "summarize",
                                          "optimize"]):
        return "shallow"
    return "none"
